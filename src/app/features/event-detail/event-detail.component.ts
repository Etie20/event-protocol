...
import { CommonModule } from '@angular/common'; // Important for date pipes etc
...
@Component({
    ...
    <div class="min-h-screen bg-slate-50 pb-24 font-sans text-slate-900">
      ...
      <!-- Scan Overlay or Header -->
      <header class="bg-white sticky top-0 z-30 safe-top border-b border-slate-100 shadow-sm">
        <div class="px-5 py-4 flex items-center justify-between">
           <div class="flex items-center gap-3">
             <button routerLink="/dashboard" class="p-2 -ml-2 rounded-full hover:bg-slate-100">
               <svg class="w-5 h-5 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" /></svg>
             </button>
             <div>
               <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Événement</p>
               <h1 class="text-lg font-bold text-slate-900 line-clamp-1">{{ event()?.title }}</h1>
             </div>
           </div>
        </div>
      </header>
      ...
      <main class="max-w-md mx-auto p-5 space-y-6">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-start justify-between">
          <div>
            ...
            <h2 class="text-lg font-bold text-slate-900 leading-tight">{{ event()?.title }}</h2>
            <p class="text-slate-500 text-sm mt-1">{{ event()?.startDate | date:'fullDate' }}</p>
            <p class="text-slate-400 text-xs mt-1">{{ event()?.locationName }}</p>
          </div>
          <div class="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 overflow-hidden">
...
export class EventDetailComponent implements OnInit {
    ...
    // Use input binding (from withComponentInputBinding)
    id = input<string>();
    ...
    ngOnInit() {
        // If id comes from route, load it
